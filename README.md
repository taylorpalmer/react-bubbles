...
A token is associated with an authorized user's login info to grant access.

- [x] What steps can you take in your web apps to keep your data secure?

Data and pages that are meant to be secure can be stored in Private Routes so unauthorized users can't view them.

- [x] Describe how web servers work.

Web servers store the code for a website or application on a remote piece of hardware that is then accessed when a user goes to view that site/app.

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

The .post method Creates data to add to the API, the .get method Reads or retrieves the information in the API, the .put method Updates the data, and the .delete method Deletes information from the API.
